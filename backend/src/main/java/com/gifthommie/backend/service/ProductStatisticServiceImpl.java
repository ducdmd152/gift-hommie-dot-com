...
public class ProductStatisticServiceImpl implements ProductStatisticService {
	...
	private ProductStatisticValue getProductStatisticFromTo(LocalDateTime startDate, LocalDateTime endDate) {
		...
		for (Integer productId : orderRepository.findTopSoldProductFromTo(startDate, endDate)) {
			Product p = productRepository.findProductById(productId, ENABLED);
			p.setSold(orderRepository.getOrderedQuantityByProductIdFromTo(productId, ORDER_SUCCESSFUL, startDate, endDate));
			p.setRating(orderRepository.getAverageRatingByProductIdFromTo(sold, startDate, endDate));
			topSale.add(p);
			sold += p.getSold();
		}

		for (Integer productId : orderRepository.findTopRatingProductFromTo(startDate, endDate)) {
